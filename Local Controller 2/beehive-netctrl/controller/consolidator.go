...
func (n monitor) Rcv(msg bh.Msg, ctx bh.RcvContext) error {
	...
	if len(stats["port"]) != 0 {
		showStats("link")
		http.SendBandwidth(1)
		http.SendTopology(1)
		http.SendMetrics(1)

		http.GetBestRoutesInterDomain()
		time.Sleep(time.Duration(10) * time.Second)
		//si existe bestRoutesGlobal hacer
		plan, err := ioutil.ReadFile("/home/controller2/go/src/github.com/kandoo/beehive-netctrl/bestRoutesGlobal.json")
		if err != nil {
...
func getPortLoss() {
	...
	for _, stat := range bodies {
		...
		if portNo != 1 && portNo != int(of13.PP_LOCAL) {
			...
			mutexPortStats.Lock()
			tmp := dictPortStats[key1]
			var txPktSrc uint64 = 0
			txPktSrc = tmp[len(tmp)-1].TxPackets

			key2 := getSwDst(stat.Node, stat.Port)
			if key2 != "" {
				tmp2 := dictPortStats[string(key2)]
				var rxPktsDst uint64 = 0
				if len(tmp2) != 0 {
					rxPktsDst = tmp2[len(tmp2)-1].RxPackets
				} 
...
func showStats(typeStat string) {
	if typeStat == "link" {
		...
		for _, k := range arrKey {
			...
			Ns := k[:del]
			Nd := k[del+2:]
			lusd2 := math.Max(linkUsedBw[k]/1000.0, 0)
			lusd := fmt.Sprintf("%.6f", lusd2)
			fmt.Printf("%2s %6s %17s %17s %14s %10s\n", Ns, Nd, lusd, netInfo[k][0], netInfo[k][1], netInfo[k][2])
		}
	}
}

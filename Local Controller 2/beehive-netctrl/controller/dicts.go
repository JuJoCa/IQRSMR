...
func (c LinksController) Rcv(msg bh.Msg, ctx bh.RcvContext) error {
	data := msg.Data().(nom.LinksToController)
	for node, _ := range data.Links {
		link := data.Links[node]
		arrLinksCon = append(arrLinksCon, link...)
	}
	numPorts = data.NumPorts
	portsBlock = data.PortBlock
	nodesRed = data.NodoRed
	return nil
}

func (c LinksController) Map(msg bh.Msg, ctx bh.MapContext) bh.MappedCells {
...
func (nd *nodeDrivers) master() driverInfo {
	...
	for _, d := range nd.Drivers {
		if d.Driver.Role == nom.DriverRoleMaster {
			return d
		}
	}
	return nd.Drivers[0]
...
func (nd *nodeDrivers) isMaster(d nom.Driver) bool {
	return nd.master().Driver.BeeID == d.BeeID
}
...
func sendToMaster(msg interface{}, node nom.UID, ctx bh.RcvContext) error {
	...
	if err != nil {
		if err == state.ErrNoSuchKey {
			fmt.Println("nom: cannot find node ", node)
			return fmt.Errorf("nom: cannot find node %s", node)
		}
		return err
	}
	nd := v.(nodeDrivers)
	ctx.SendToBee(msg, nd.master().Driver.BeeID)
	return nil
}
...
func (f *flow) updateStats(stats nom.FlowStats) {
	...
	f.DurationNsec = stats.DurationNsec
	f.Bytes = stats.Bytes

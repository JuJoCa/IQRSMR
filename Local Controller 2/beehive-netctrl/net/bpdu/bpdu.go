...
func NewSTPTLVWithBuf(b []byte) LinkDiscoveryTLV {
	return LinkDiscoveryTLV{packet.Packet{Buf: b}}
}
...
//agregando spanning tree protocol wireshark
func (this *LinkDiscoveryTLV) SetRootIdent(s [6]uint8, p uint16) {
	offset := this.RootIdentOffset()
	binary.BigEndian.PutUint16(this.Buf[offset-2:], p)
	for _, e := range s {
...
func (this LinkDiscoveryTLV) RootIdentOffset() int {
	offset := 7
	return offset
}

func (this *LinkDiscoveryTLV) SetPathCost(c uint16) {
	offset := this.PathCostOffset()
	binary.BigEndian.PutUint16(this.Buf[offset:], c)
	offset += 4
}

func (this LinkDiscoveryTLV) PathCostOffset() int {
	offset := 15
	return offset
}

func (this *LinkDiscoveryTLV) SetBridgeIdent(s [6]uint8, p uint16) {
	offset := this.RootIdentOffset2()
	binary.BigEndian.PutUint16(this.Buf[offset-2:], p)
	for _, e := range s {
		this.Buf[offset] = byte(e)
		offset++
...
func (this LinkDiscoveryTLV) RootIdentOffset2() int {
	offset := 19
	return offset
}

func (this *LinkDiscoveryTLV) SetDelay() {
	offset := this.DelayOffset()
	binary.BigEndian.PutUint16(this.Buf[offset:], 15)
	//15 segundo de delay
}

func (this LinkDiscoveryTLV) DelayOffset() int {
	offset := 32
	return offset
}
...
func NewSTPProtocolWithBuf(b []byte) LinkDiscoveryProtocol {
...
func (this LinkDiscoveryProtocol) Clone() (LinkDiscoveryProtocol, error) {
	...
	return NewSTPProtocolWithBuf(newBuf.Bytes()), nil
}
...
func (c *LinkDiscoveryProtocolConn) ReadLinkDiscoveryProtocols(pkts []LinkDiscoveryProtocol) (int, error) {
	...
	n := 0
	for i := range pkts {
		p := NewSTPProtocolWithBuf(c.buf[s:])
...
func ToLinkDiscoveryProtocol(p ethernet.Ethernet) (LinkDiscoveryProtocol, error) {
	if !IsLinkDiscoveryProtocol(p) {
		return NewSTPProtocolWithBuf(nil), errors.New("Cannot convert to lldp.LinkDiscoveryProtocol")
	}

	return NewSTPProtocolWithBuf(p.Buf), nil
}
...
func (this *ChassisTLV) SetTypeAndLen(t uint16) {
	offset := this.TypeAndLenOffset()
	//fmt.Println("t: ", t, " offset: ", offset, " bufa: ", this.Buf)
	binary.BigEndian.PutUint16(this.Buf[offset:], t)
	//fmt.Println("Buf: ", this.Buf)
	offset += 1
}

func (this *ChassisTLV) SetTypeAndLen2(t uint16) {
	offset := this.TypeAndLenOffset2()
	//fmt.Println("txd: ", t, " offset: ", offset, " bufa: ", this.Buf)
	binary.BigEndian.PutUint16(this.Buf[offset:], t)
	//fmt.Println("Bufxd: ", this.Buf)
	offset += 1
}
...
func (this ChassisTLV) TypeAndLenOffset2() int {
	offset := 1
	return offset
}
...
func (this *ChassisTLV) SetSubtype0(s uint8) {
	offset := this.SubtypeOffset0()
	//fmt.Println("this: ", this.Buf, " s: ", s, " offset: ", offset, " byte(s): ", byte(s))
	this.Buf[offset] = byte(s)

	offset += 1
}

func (this *ChassisTLV) SetSubtype1(s uint8) {
	offset := this.SubtypeOffset1()
	this.Buf[offset] = byte(s)
	offset += 1
}
...
func (this ChassisTLV) SubtypeOffset0() int {
	offset := 0
	return offset
}

func (this ChassisTLV) SubtypeOffset1() int {
	offset := 1
	return offset
}
...
func (this *ChassisMacTLV) Init() {
	...
	// Invariants.
	//Se eliminino this.SetTypeAndLen(uint16()) y se cambio por this.SetSubtype0(uint8())
	this.SetSubtype0(uint8(66))
	this.SetSubtype1(uint8(66))
	this.SetSubtype(uint8(3))
}

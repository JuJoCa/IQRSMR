...
func (c *ofConn) doWrite(done chan struct{}, stop chan struct{}) {
	...
	for {
		...
		if !written {
			...
			case msg = <-c.wCh:
			case <-stop:
				//fmt.Println("return2wr")
				return
			default:
				if c.wErr = c.HeaderConn.Flush(); c.wErr != nil {
					//fmt.Println("return3wr")
					//return
					continue
				}
				written = false
				//fmt.Println("continuewr")
				continue
			}
		}
		...
		if c.wErr != nil {
			//fmt.Println("return4wr")
			return
		}

		if err != nil {
			//fmt.Println("errwr: ", err)
			//glog.Errorf("ofconn: Cannot convert NOM message to OpenFlow: %v",
			//	err)
		}
		written = true
	}
	//fmt.Println("Salio de write")
}

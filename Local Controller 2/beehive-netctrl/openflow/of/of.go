...
func (c *HeaderConn) ReadHeader() (Header, error) {
	//fmt.Println("Entra a readHeader")
	pkts := make([]Header, 1)
	//fmt.Println("pktsAntes: ", pkts)
	_, err := c.ReadHeaders(pkts)
	if err != nil {
		return NewHeader(), err
	}
	//fmt.Println("pktsDespues: ", pkts)
	return pkts[0], nil
}

func (c *HeaderConn) ReadHeaders(pkts []Header) (int, error) {
	//fmt.Println("len(c.buf): ", len(c.buf), " c.offset: ", c.offset)
	// c.offset por lo general es 0
	...
	//fmt.Println("c.bufates: ", c.buf[:20])
	r, err := c.Conn.Read(c.buf[c.offset:]) // r=32
...
// colocamos esto que agrega datos(time) a el echo reply
func (this *Header) SetData(t string) {
	offset := this.DataOffset()
	timeByte := []byte(t)
	//fmt.Println("timeByte: ", timeByte, " buf: ", this.Buf)
	for i := 0; i < len(timeByte); i++ {
		this.Buf[i+offset] = timeByte[i]
	}
	//fmt.Println("thisBuf: ", this.Buf)
	//this.Buf[offset:] = append(this.Buf[offset:], timeByte...)
	offset += len(timeByte)
}

func (this Header) DataOffset() int {
	offset := 8
	return offset
}

func (this *Header) SetTypeMulti(t uint8) {
	offset := this.TypeOffsetmulti()
	this.Buf[offset] = byte(t)
	offset++
}
...
func (this Header) TypeOffsetmulti() int {
	offset := 9
	return offset
}

...
func (this *FlowMod) SetMatch(m Match) {
	...
	offset := this.MatchOffset()
	//fmt.Println("match: ", m, " offset: ", offset, " Buf: ", this.Buf[:], "m.Buf: ", m.Buf[:m.Size()], "m.size(): ", m.Size())
	copy(this.Buf[offset:], m.Buf[:m.Size()])
	//fmt.Println("m.size: ", m.Size())
	offset += m.Size()
	//fmt.Println("offsetDespues: ", offset, " Buf: ", this.Buf[:])
}
...
func IsFlowStatsReply(p StatsReply) bool {
	return p.StatsType() == 17 && true
}

func (this FlowStatsReply) FlowStats() []FlowStats {
	offset := this.FlowStatsOffset()
	//fmt.Printf("offset es: ", offset)
	//fmt.Printf("\n")
	packet_size := this.Size()
	//fmt.Printf("packet_size es: ", packet_size)
	//fmt.Printf("\n")
	size := packet_size - offset
	count := this.Size() - offset
	var res []FlowStats

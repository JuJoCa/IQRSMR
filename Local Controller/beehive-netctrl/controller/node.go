...
var nodosCtrl = make(map[nom.UID]string)
...
func (h nodeConnectedHandler) Rcv(msg bh.Msg, ctx bh.RcvContext) error {
	...
	k := string(nc.Node.ID) // k es el numero del nodo (ej: 1, 2, 3, 4, ...)
	n := nodeDrivers{
		Node: nc.Node,
	}
	...
	db := nc.Driver.BeeID
	// Para saber si esta conectado el nodo a controlador root o controlador local de cada dominio
	if len(n.Drivers) == 0 {
...
func (h nodeDisconnectedHandler) Rcv(msg bh.Msg, ctx bh.RcvContext) error {
	...
	if ndd.Driver.Role == nom.DriverRoleMaster {
		...
		nd.Drivers[0].Driver.Role = nom.DriverRoleMaster
		ctx.SendToBee(nom.ChangeDriverRole{
			Node:       nd.Node.UID(),
			Role:       nom.DriverRoleMaster,
			Generation: gen,
		}, nd.master().Driver.BeeID)
	}
...
func (h roleUpdateHandler) Rcv(msg bh.Msg, ctx bh.RcvContext) error {
	...
	for i := range n.Drivers {
		if n.Drivers[i].Driver.BeeID == ru.Driver.BeeID {
			found = true
			n.Drivers[i].Driver.Role = ru.Driver.Role
			break
		}
	}
